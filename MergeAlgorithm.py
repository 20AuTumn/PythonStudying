# 병합 알고리즘(Merge Algorithm): 오름차순으로 정렬되어 있는 정수배열을 하나로 병합
...
#[2] Process: Merge
while (i < M and j < N): # 둘 중 하나라도 배열의 끝에 도달할 때 까지 (반복)
    if first[i] < second[j]: # 작은 값을 merge 배열에 저장
        merge[k] = first[i]; k += 1; i += 1;
    else:
        merge[k] = second[j]; k += 1; j += 1;
while (i < M): # 첫번째 배열이 끝까지 도달할 때 까지 (반복)
    merge[k] = first[i]; k += 1; i += 1;
while (j < N): # 두번째 배열이 끝까지 도달할 때 까지 (반복)
    merge[k] = second[j]; k += 1; j += 1;
